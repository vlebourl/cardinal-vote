...
class InputSanitizer:
    ...
    def sanitize_email(cls, email: str) -> str:
        ...
        # Normalize email case according to RFC standards:
        # - Domain part (after @) is case-insensitive, so normalize to lowercase
        # - Local part (before @) should preserve original case for compatibility
        # This approach ensures consistent authentication while respecting RFC standards
        if "@" in email:
    ...
    def sanitize_password(cls, password: str) -> str:
        ...
        # Don't allow common dangerous patterns that could cause issues
        dangerous_patterns = [
            "\x00",  # Null byte
            "\r\n",  # CRLF injection
            "\n",  # Line feed
