...
class PublicVotingApp {
    ...
    showVotingScreen() {
        ...
        this.announceToScreenReader(`Écran de vote. Option ${this.currentOptionIndex + 1} sur ${this.voteData.options.length}`);
    }
    ...
    displayCurrentOption() {
        ...
        if (previousRating !== undefined) {
            this.announceToScreenReader(`Option ${this.currentOptionIndex + 1} sur ${this.voteData.options.length}: ${option.title}. Note actuelle : ${this.getRatingLabel(previousRating)}`);
        } else {
            this.announceToScreenReader(`Option ${this.currentOptionIndex + 1} sur ${this.voteData.options.length}: ${option.title}. Aucune note attribuée`);
        }
    }
    ...
    updateProgressBarAria() {
        ...
        if (progressBar && this.voteData && this.voteData.options) {
            ...
            progressBar.setAttribute('aria-valuetext', `Option ${this.currentOptionIndex + 1} sur ${this.voteData.options.length}`);
        }
    }
    ...
    showReviewScreen() {
        ...
        const unratedOptions = this.voteData.options.filter(option =>
            !this.responses.hasOwnProperty(option.id)
        );
    ...
    async submitVote() {
        ...
        try {
            ...
            const missingOptions = this.voteData.options.filter(option =>
                !this.responses.hasOwnProperty(option.id)
            );
