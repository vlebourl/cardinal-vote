...
class DashboardManager {
  ...
  initializeEventListeners() {
    ...
    this.searchInput.addEventListener(
      'input',
      this.debounce(() => this.filterVotes(), 300)
    )
  ...
  async loadDashboardData() {
    try {
      await Promise.all([this.loadStats(), this.loadRecentActivity(), this.loadPopularVotes()])
    } catch (error) {
  ...
  updateVotesList() {
    ...
    this.votesList.innerHTML = this.votes
      .map(
        vote => `
