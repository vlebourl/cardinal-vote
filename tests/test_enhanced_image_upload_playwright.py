...
class TestEnhancedImageUploadInterface:
    ...
    def create_test_image(
        self, format_type: str = "PNG", size: tuple = (800, 600)
    ) -> Path:
    ...
    async def test_image_upload_interface_display(self, authenticated_page: Page):
        ...
        await page.wait_for_selector(
            "#voteCreationModalScrim.md-dialog-scrim-visible", timeout=10000
        )

        # Check default options have image upload interfaces
        upload_containers = await page.query_selector_all(".option-image-upload")
        assert len(upload_containers) >= 2, (
            "Should have image upload for default options"
        )
        ...
        for i, container in enumerate(upload_containers[:2]):
            ...
            assert icon_text == "cloud_upload", (
                f"Upload icon should be correct for option {i}"
            )
            assert "Click or drag image here" in text_content, (
                f"Upload text should be correct for option {i}"
            )
            assert "PNG, JPG, GIF, WebP" in hint_content, (
                f"Upload hint should be correct for option {i}"
            )
    ...
    async def test_image_file_upload_success(self, authenticated_page: Page):
        ...
        try:
            ...
            await page.wait_for_selector(
                f'#preview-{option_id}[style*="block"]', timeout=10000
            )

            # Verify upload progress was shown
            upload_container = await page.query_selector(
                f'.option-image-upload[data-option-id="{option_id}"]'
            )
            _ = await upload_container.get_attribute("class")  # Check container exists

            # Check that upload area is hidden after successful upload
            upload_style = await upload_container.get_attribute("style")
            assert "display: none" in upload_style, (
                "Upload container should be hidden after successful upload"
            )
            ...
            assert "test_image_png.png" in filename_text, (
                "Filename should be displayed correctly"
            )

            # Verify details (size and dimensions)
            details_text = await image_details.inner_text()
            assert "KB" in details_text or "MB" in details_text, (
    ...
    async def test_image_upload_validation(self, authenticated_page: Page):
        ...
        try:
            ...
            await page.wait_for_selector(
                f'#error-{option_id}[style*="flex"]', timeout=5000
            )
            ...
            assert "valid image file" in error_text.lower(), (
                f"Should show invalid file type error: {error_text}"
            )
    ...
    async def test_image_remove_functionality(self, authenticated_page: Page):
        ...
        try:
            ...
            remove_btn = await page.query_selector(
                f"#preview-{option_id} .remove-image-btn"
            )
            ...
            upload_style = await upload_container.get_attribute("style")
            assert "display: block" in upload_style, (
                "Upload container should be visible after remove"
            )

            # Verify file input is cleared
            file_input_value = await file_input.get_attribute("value")
            assert file_input_value == "" or file_input_value is None, (
    ...
    async def test_multiple_option_image_uploads(self, authenticated_page: Page):
        ...
        upload_containers = await page.query_selector_all(".option-image-upload")
        assert len(upload_containers) >= 3, (
            "Should have at least 3 options with upload interfaces"
        )
        ...
        try:
            for i, test_image in enumerate(test_images):
                ...
                preview_container = await page.query_selector(f"#preview-{option_id}")
                filename_element = await preview_container.query_selector(
                    ".image-filename"
                )
                filename = await filename_element.inner_text()

                expected_format = "png" if i == 0 else "jpeg"
                assert expected_format in filename.lower(), (
                    f"Option {i} should show correct filename"
                )

            # Verify third option still shows upload interface
            third_container = upload_containers[2]
            _ = await third_container.get_attribute(
                "data-option-id"
            ...
            assert "display: none" not in (third_upload_style or ""), (
                "Third option should still show upload interface"
            )
    ...
    async def test_vote_creation_with_images(self, authenticated_page: Page):
        ...
        try:
            ...
            is_success = (
                "/vote-preview/" in current_url
                or "Vote created successfully" in await page.content()
            )
    ...
    async def test_image_upload_accessibility(self, authenticated_page: Page):
        ...
        assert file_input_id == f"image-{option_id}", (
            "File input should have correct ID"
        )
        assert ".png,.jpg,.jpeg,.gif,.webp" in accept_attr, (
            "File input should have correct accept attribute"
        )
        ...
        try:
            ...
            _ = await remove_btn.get_attribute("aria-label")  # Check accessibility
